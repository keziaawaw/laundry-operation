...
class TestBillingAPI:
    ...
    def test_calculate_cost_regular(self, client):
        ...
        response = client.post(
            "/billing/calculate",
            json={"weight_kg": 5.0, "service_type": "regular", "note": None}
        )
    ...
    def test_calculate_cost_express(self, client):
        ...
        response = client.post(
            "/billing/calculate",
            json={"weight_kg": 3.0, "service_type": "express"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_calculate_cost_delicate(self, client):
        ...
        response = client.post(
            "/billing/calculate",
            json={"weight_kg": 2.0, "service_type": "delicate"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_calculate_cost_unknown_service_type(self, client):
        ...
        response = client.post(
            "/billing/calculate",
            json={"weight_kg": 4.0, "service_type": "unknown"}
        )
        assert response.status_code == 200
        data = response.json()
        assert data["multiplier"] == 1.0
    ...
    def test_calculate_cost_with_note(self, client):
        ...
        response = client.post(
            "/billing/calculate",
            json={"weight_kg": 5.0, "service_type": "regular", "note": "Handle with care"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_calculate_cost_edge_cases(self, client):
        ...
        response = client.post(
            "/billing/calculate",
            json={"weight_kg": 0.0, "service_type": "regular"}
        )
        assert response.status_code == 200
        ...
        response = client.post(
            "/billing/calculate",
            json={"weight_kg": 0.1, "service_type": "regular"}
        )
        assert response.status_code == 200
        ...
        response = client.post(
            "/billing/calculate",
            json={"weight_kg": 100.0, "service_type": "express"}
        )
        assert response.status_code == 200
